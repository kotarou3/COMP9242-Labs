...
class FlatFileSystem : public FileSystem {
    ...
    private:
        async::future<struct stat> _statLoop(const std::string& pathname, std::vector<std::unique_ptr<FileSystem>>::iterator fs);
        async::future<std::shared_ptr<File>> _openLoop(const std::string& pathname, OpenFlags flags, std::vector<std::unique_ptr<FileSystem>>::iterator fs);
...
class FlatDirectory : public Directory {
    public:
        ...
        virtual async::future<ssize_t> getdents(memory::UserMemory dirp, size_t length);
